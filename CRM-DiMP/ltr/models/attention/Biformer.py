...
class BiLevelRoutingAttention_nchw(nn.Module):
    ...
    * different pooling behavior (count_include_pad=False)
    ...
    def forward(self, x:Tensor, ret_attn_mask=False):
        ...
        # import math
        # h=math.ceil(H/region_size[0])
        # w=math.ceil(W/region_size[1])
        # # xr = torch.gather(x, dim=2, index=idx_r)  # xyl 20231116，可视化路由矩阵
        # # show_tensor(xr.squeeze(0).reshape(-1,h,w),7,'xr')  # xyl 20231116，可视化路由矩阵
        # show_tensor(idx_r.sum(dim=1).squeeze(0).reshape(-1,h,w),8,'idx_r')  # xyl 20231116，可视化路由矩阵
        # show_tensor(a_r,4,'a_r')  # xyl 20231116，可视化路由矩阵
        # show_tensor(q_r,5,'q_r')  # xyl 20231116，可视化路由矩阵
        # show_tensor(k_r,6,'k_r')  # xyl 20231116，可视化路由矩阵
        # show_tensor(v.sum(dim=1),7,'v')  # xyl 20231116，可视化路由矩阵
