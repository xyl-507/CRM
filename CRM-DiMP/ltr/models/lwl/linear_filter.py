...
class LinearFilter(nn.Module):
    ...
    def get_filter(self, feat, train_label, train_sw, num_objects=None, *args, **kwargs):  # train_sw 空间权重
        """ Get the initial target model parameters given the few-shot labels """
        if num_objects is None:
            weights = self.filter_initializer(feat, train_label)
        else:
            weights = self.filter_initializer(feat, train_label)  # Initializes a target model with zeros.
            weights = weights.repeat(1, num_objects, 1, 1, 1)

        if self.filter_optimizer is not None:
            weights, weights_iter, losses = self.filter_optimizer(TensorList([weights]), feat=feat, label=train_label,  # 小样本学习器
