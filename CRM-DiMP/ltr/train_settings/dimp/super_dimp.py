...
def run(settings):
    ...
    optimizer = optim.Adam([{'params': actor.net.classifier.filter_initializer.parameters(), 'lr': 5e-5},
                            ...
                            {'params': actor.net.bb_regressor.parameters(), 'lr': 5e-4},  # pytracking 1e-3
                            {'params': actor.net.feature_extractor.layer3.parameters(), 'lr': 5e-5},  # pytracking 2e-5
                            {'params': actor.net.crm.parameters(), 'lr': 5e-4}],  # mvt
                           lr=2e-6)  # 没有单独指定学习率的，就是用这个学习率。 pytracking 是 2e-4
