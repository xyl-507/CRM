...
class DiMP(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        scores_raw = self.classify_target(test_x)  # 利用滤波器和搜索特征进行卷积得到分类分数
        test_x_ = self.net.mvt(test_x)
        scores_raw2 = self.classify_target2(test_x_)  # 利用滤波器2和搜索特征进行卷积得到分类分数
        # ----------------------------------------------------------------------------------------------multi-pytracking
    ...
    def track2(self, image, info: dict = None) -> dict:
        ...
        test_x_ = self.net.mvt(test_x)
        scores_raw = self.classify_target(test_x_)  # 利用滤波器2和搜索特征进行卷积得到分类分数
